...
게임 요소 분석과 정량 게이트는 [`docs/game-completeness.md`](docs/game-completeness.md), CI 강제 검사는 [`tool/verify_game.dart`](tool/verify_game.dart)에 있습니다. 게이트는 콘텐츠·분기·결정론·시각·자산·추적성·배포·입력 계약·저장 연속성·종료 안전성 10개 차원을 계산하며 95% 미만이면 실패합니다. SSOT 검사 → 해시 매니페스트 → 정적 분석 → 상태/Golden 테스트 → Wasm 빌드 순서가 모두 통과해야 저장소 변경이 검증됩니다.
