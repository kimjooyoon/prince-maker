...
Map<String, dynamic> play(StoryPort story, Map<String, dynamic> activity) {
  ...
  while (session.world.progress[0]!.week < story.endingWeek) {
    session.choose(ActivityChosen(activity['stat'], activity['delta'], activity['coins'], activity['fatigue'], label: activity['label']));
    ...
    final choice = choices.firstWhere((choice) => (choice['requiresStat'] == null || (session.world.stats[0]!.values[choice['requiresStat']] ?? 0) >= (choice['requiresMin'] as int? ?? 0)) && (choice['requiresBondId'] == null || (session.world.progress[0]!.bonds[choice['requiresBondId']] ?? 0) >= (choice['requiresBondMin'] as int? ?? 0)) && (choice['requiresFlag'] == null || session.world.progress[0]!.flags[choice['requiresFlag']] == true));
  ...
  return {'ending': ending['id'], 'bonds': Map.of(progress.bonds), 'goals': progress.milestones.values.where((value) => value).length, 'trace': List.of(progress.trace)};
}

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();
  test('five SSOT schedule policies produce measurable route variety', () async {
    final source = jsonDecode(await rootBundle.loadString('story/story.json')) as Map<String, dynamic>;
    final story = JsonStoryAdapter(source), activities = (source['activities'] as List).cast<Map<String, dynamic>>();
    final routes = activities.map((activity) => play(story, activity)).toList();
    final endings = routes.map((route) => route['ending']).toSet();
    final signatures = routes.map((route) => '${route['ending']}|${route['bonds']}|${route['goals']}').toSet();
    expect(routes.length, 5);
    expect(endings.length, greaterThanOrEqualTo(3));
    expect(signatures.length, greaterThanOrEqualTo(3));
    expect(routes.every((route) => (route['trace'] as List).length >= 12), isTrue);
    print('GAMEPLAY_METRICS_OK: policies=${routes.length} distinctEndings=${endings.length} distinctSignatures=${signatures.length}');
  });
}
