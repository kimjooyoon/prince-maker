...
void main() {
  ...
  Future<Map<String, dynamic>> play(String stat, int persona) async {
    ...
    while (session.world.progress[0]!.week < story.endingWeek) {
      ...
      if (events.isNotEmpty) {
        ...
        session.chooseEvent(StoryChoiceMade(choice['stat'], choice['delta'], choice['coins'], choice['label'], bondId: choice['bondId'], bondDelta: choice['bondDelta'], rivalId: choice['rivalId'], rivalDelta: choice['rivalDelta'] ?? 0, requiresStat: choice['requiresStat'], requiresMin: choice['requiresMin'] ?? 0, requiresBondId: choice['requiresBondId'], requiresBondMin: choice['requiresBondMin'] ?? 0, requiresFlag: choice['requiresFlag'], setsFlag: choice['setsFlag']));
      }
    }
  ...
  test('canonical SSOT completes a deterministic 12-week route', () async {
    ...
    while (session.world.progress[0]!.week < story.endingWeek) {
      ...
      if (events.isNotEmpty) {
        final choices = (events.first['choices'] as List).cast<Map<String, dynamic>>();
        final choice = choices.firstWhere((c) => (c['requiresStat'] == null || (session.world.stats[0]!.values[c['requiresStat']] ?? 0) >= (c['requiresMin'] as int? ?? 0)) && (c['requiresBondId'] == null || (session.world.progress[0]!.bonds[c['requiresBondId']] ?? 0) >= (c['requiresBondMin'] as int? ?? 0)) && (c['requiresFlag'] == null || session.world.progress[0]!.flags[c['requiresFlag']] == true));
        session.chooseEvent(StoryChoiceMade(choice['stat'], choice['delta'], choice['coins'], choice['label'], bondId: choice['bondId'], bondDelta: choice['bondDelta'], rivalId: choice['rivalId'], rivalDelta: choice['rivalDelta'] ?? 0, requiresBondId: choice['requiresBondId'], requiresBondMin: choice['requiresBondMin'] ?? 0, requiresFlag: choice['requiresFlag'], setsFlag: choice['setsFlag'], line: choice['line']));
      }
    }
  ...
  test('every authored ending and event choice is reachable under its contract', () async {
    ...
    for (final event in story.events) {
      for (final raw in (event['choices'] as List).cast<Map<String, dynamic>>()) {
        ...
        if (requiredBond != null) session.world.progress[0]!.bonds[requiredBond] = raw['requiresBondMin'] as int;
        final requiredFlag = raw['requiresFlag'] as String?;
        if (requiredFlag != null) session.world.progress[0]!.flags[requiredFlag] = true;
        final before = session.world.stats[0]!.values[raw['stat'] as String]!;
        session.chooseEvent(StoryChoiceMade(raw['stat'], raw['delta'], raw['coins'], raw['label'], bondId: raw['bondId'], bondDelta: raw['bondDelta'], rivalId: raw['rivalId'], rivalDelta: raw['rivalDelta'] ?? 0, requiresStat: required, requiresMin: raw['requiresMin'] ?? 0, requiresBondId: requiredBond, requiresBondMin: raw['requiresBondMin'] ?? 0, requiresFlag: requiredFlag, setsFlag: raw['setsFlag'], line: raw['line']));
