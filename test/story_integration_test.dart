...
void main() {
  ...
  Future<Map<String, dynamic>> play(String stat, int persona) async {
    ...
    while (session.world.progress[0]!.week < story.endingWeek) {
      ...
      if (events.isNotEmpty) {
        ...
        session.chooseEvent(StoryChoiceMade(choice['stat'], choice['delta'], choice['coins'], choice['label'], bondId: choice['bondId'], bondDelta: choice['bondDelta'], rivalId: choice['rivalId'], rivalDelta: choice['rivalDelta'] ?? 0, requiresStat: choice['requiresStat'], requiresMin: choice['requiresMin'] ?? 0, requiresBondId: choice['requiresBondId'], requiresBondMin: choice['requiresBondMin'] ?? 0));
      }
    }
  ...
  test('canonical SSOT completes a deterministic 12-week route', () async {
    ...
    while (session.world.progress[0]!.week < story.endingWeek) {
      ...
      if (events.isNotEmpty) {
        final choices = (events.first['choices'] as List).cast<Map<String, dynamic>>();
        final choice = choices.firstWhere((c) => (c['requiresStat'] == null || (session.world.stats[0]!.values[c['requiresStat']] ?? 0) >= (c['requiresMin'] as int? ?? 0)) && (c['requiresBondId'] == null || (session.world.progress[0]!.bonds[c['requiresBondId']] ?? 0) >= (c['requiresBondMin'] as int? ?? 0)));
        session.chooseEvent(StoryChoiceMade(choice['stat'], choice['delta'], choice['coins'], choice['label'], bondId: choice['bondId'], bondDelta: choice['bondDelta'], rivalId: choice['rivalId'], rivalDelta: choice['rivalDelta'] ?? 0, requiresBondId: choice['requiresBondId'], requiresBondMin: choice['requiresBondMin'] ?? 0, line: choice['line']));
      }
    }
  ...
  test('every authored ending and event choice is reachable under its contract', () async {
    ...
    for (final event in story.events) {
      for (final raw in (event['choices'] as List).cast<Map<String, dynamic>>()) {
        ...
        session.chooseEvent(StoryChoiceMade(raw['stat'], raw['delta'], raw['coins'], raw['label'], bondId: raw['bondId'], bondDelta: raw['bondDelta'], rivalId: raw['rivalId'], rivalDelta: raw['rivalDelta'] ?? 0, requiresStat: required, requiresMin: raw['requiresMin'] ?? 0, requiresBondId: requiredBond, requiresBondMin: raw['requiresBondMin'] ?? 0, line: raw['line']));
