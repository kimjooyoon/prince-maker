...
void main() {
  testWidgets('home footer opens the archive from its full tap target',
      (tester) async {
    ...
    await tester.pump();
    expect(find.byKey(const ValueKey('7-1-0-0')), findsOneWidget);
  });

  testWidgets('home opens the twenty-character archive', (tester) async {
    ...
    expect(find.byKey(const ValueKey('7-1-0-0')), findsOneWidget);
    await expectLater(
        find.byType(Game), matchesGoldenFile('goldens/character-roster.png'));
    await tester.tapAt(const Offset(650, 40));
    await tester.pump();
    expect(find.byKey(const ValueKey('7-1-0-0-en')), findsOneWidget);
    await expectLater(find.byType(Game),
        matchesGoldenFile('goldens/character-roster-en.png'));
  });
}
