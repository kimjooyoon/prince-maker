...
UI는 `GameSession`의 명령만 호출하고 JSON 파일이나 저장 구현을 직접 알지 않는다. 스토리는 `StoryPort`와 결정론적 `resolveEnding`, 저장은 `SavePort`로 분리해 향후 IndexedDB·파일 저장·서버 동기화 어댑터를 교체할 수 있다. 현재 기본 저장 어댑터는 단일 사용자 로컬 플레이를 위한 메모리 어댑터이며, export/import는 동료 유대까지 포함한 versioned `lumen-save-v3` JSON snapshot으로 제공한다.
