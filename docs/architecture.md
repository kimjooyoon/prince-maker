...
성격×동료 조합은 [`story/story.jsonl#personalityCompanionRoutes`](../story/story.jsonl)의 3×3 matrix를 [`resolvePersonalityCompanionRoute`](../lib/game_core.dart)가 순수 투영한다. 매칭된 조합의 승인된 사건 선택은 `PersonalityCompanionResonanceApplied` ECS event를 FIFO 큐에 넣어 동료 유대 +1과 `resonance:<persona>:<companion>` trace를 남기며, 같은 유대 임계가 엔딩 epilogue route set을 결정한다. [`test/personality_companion_route_test.dart`](../test/personality_companion_route_test.dart)와 관계 archive Golden이 core→ending→Canvas 연결을 함께 고정한다.
