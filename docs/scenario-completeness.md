...
| 장면 | 도입·중반 사건·사이드 장면·막 관계 장면·관계 상태·상태별 후속 대화·장소 발견·관계 긴장·관계 중재·외출·유대·기억·계승 게이트·계승 프로필·동행 선택/잠금/혼합 상태·엔딩·공통 UI 상태·Star Cellar Golden·design system Golden | 109 Golden, 그중 16막 canonical 사건 16/16·결산 16/16·관계 장면 16/16, home/closure relationship state/follow-up rendering, page 11 relationship archive ko/en, page 13 companion choice/record/locked/mixed ko/en, canonical 4주차·48주차 handoff 사건, 6 location discovery flags/traces, 24 side-scene records·10 activity mini-events·18 companion scenes·36 companion choices·18 ending variants, rival loss/mediation, outing/bond/memory/legacy feedback, fatigue risk/recovery forecast with deterministic recovery days, pre-commit authored memory impact title and detail, ending picker ko/en with explicit target ending forecast and selected next-run profile, next-run home lineage feedback, six-state Canvas UI matrix, design system ko/en variant matrix, Star Cellar ko/en room dodge/collection loop, 3 companion epilogue endings | 다음 회차 프로필 선택이 `GameSession.legacyId`와 후속 replay trace 및 홈 피드백을 여는 Golden 확장 |
