...
현재 12주 prototype은 4막, 8사건, 4계절 목표로 이 계약을 고정한다. 각 사건 선택은 `stat`, `coins`, `bondDelta`, `line`을 가지며 `GameSession`과 replay trace를 통해 검증한다.

### 상태·관계·엔딩 완전성 행렬

| 축 | 최소 표본 | 현재 증거 | 다음 확장 기준 |
| --- | ---: | --- | --- |
| 시간/막 | 4막, 막당 사건 1개 이상 | `progression` 4개, 사건 8개 | 막마다 공개·압력·선택·결산 Golden 추가 |
