...
게임성 KPI는 story/story.json의 gameplayKpis를 원천으로 한다. choiceImpactRate는 결과를 바꾸는 authored choice 비율, eventDivergenceRate는 사건마다 최소 두 개의 효과 벡터가 있는 비율, multiAxisImpactRate는 하나의 선택이 두 개 이상의 수치 축을 움직이는 비율, gatedChoices는 조건부 선택 수다. tool/verify_gameplay_fun.dart가 feedback.png Golden까지 확인해 purity 축에 fail-closed로 연결한다.
