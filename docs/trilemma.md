...
| 축 | 플레이어에게 의미 | 자동 증거 |
| --- | --- | --- |
| 완전성 | 콘텐츠·분기·저장·화면·대사가 끝까지 이어지는가 | `verify_game.dart` 14차원 + 120분 분량 계약, 시나리오 8축, 2,048개 실제 분기 벡터(최소 2,000), 122,880개 route input 계약, Golden 62종(도입·중반·48주 handoff·terminal·운명 기록 ko/en·실제 16막 chapter closure·시스템 승인/거절 영수증), 4개 장소 발견 flag/trace, 3개 회차 계승 프로필, 6개 나비효과 기록·9개 동료 퀘스트 단계, 노아·3명 동료 registry와 94개 사건 speaker portrait binding, 455 locale keys 이상 × 한국어·English 계약, 엔딩 원인·미달 목표 회고 |
...
현재 시각 책임 증적은 62개 Golden과 463개 locale catalog key(SSOT minimum 455)이며, 94개 사건 선택의 speaker portrait는 character registry와 asset/frame key를 통해 재현된다. `system-receipt.png`는 승인·거절 trace의 owner·contract·rule·decisionHash를 Canvas에서 재현한다.
