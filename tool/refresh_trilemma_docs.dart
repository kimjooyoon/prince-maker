...
String render(String source) {
  final goldens = Directory('test/goldens')
      ...
      .length;
  final localeKeys = File(
    'story/locales/ko.jsonl',
  ).readAsLinesSync().where((line) => line.contains('"type":"entry"')).length;
  ...
  output = output
      ...
      .replaceAll(
        RegExp(r'Canvas Golden 증적은 \d+장'),
        'Canvas Golden 증적은 ${goldens}장',
      )
      .replaceAll(
        RegExp(r'ko/en \d+ locale keys'),
        'ko/en ${localeKeys} locale keys',
      )
      .replaceAll(
        RegExp(r'\d+개 locale catalog key'),
        '${localeKeys}개 locale catalog key',
      )
      .replaceAll(
        RegExp(r'\d+ locale catalog key'),
        '${localeKeys} locale catalog key',
      )
      .replaceAll(
        RegExp(r'\d+ locale minimum key'),
        '${localeKeys} locale minimum key',
      )
      .replaceAll(
      ...
      .replaceAll(
        '3개 성격별 page 1 상반신 Golden, 16개 closure Golden',
  ...
  if (!output.contains('엔딩 뒤 다음 회차 계승 선택 Golden')) {
    output = output.replaceAll(
      '3개 회차 계승 프로필',
      '3개 회차 계승 프로필·엔딩 뒤 다음 회차 계승 선택 Golden(ko/en)',
    );
  }
  if (!output.contains('엔딩 뒤 계승 프로필 선택 Golden')) {
    output = output.replaceAll(
      '3개 계승 프로필·3개 프로필별 선택 보정',
  ...
  output = output
      ...
      .replaceAll(
        '세 성격의 관계 공명 archive 화면과 활동 forecast·활동 회고 일지·세 성격별',
        '세 성격의 관계 공명 archive 화면과 활동 forecast·활동 회고 일지(ko/en)·세 성격별',
      )
      .replaceAll(
        '세 성격의 관계 공명 archive 화면과 세 성격별 page 1 상반신 일러스트 화면을 추가해',
        '세 성격의 관계 공명 archive 화면과 활동 forecast·활동 회고 일지(ko/en)·세 성격별 page 1 상반신 일러스트 화면을 추가해',
      );
  if (!output.contains('활동 회복 창')) {
    output = output
        .replaceAll('활동 피로 위험·활동 회고', '활동 피로 위험·활동 회복 창·활동 회고')
        .replaceAll(
          '활동 피로 위험 Golden·활동 회고',
          '활동 피로 위험 Golden·활동 회복 창 Golden·활동 회고',
        )
        .replaceAll(
          'fatigue risk/recovery forecast',
  ...
  output = output
      .replaceAll(
        '`replayChecksum`, signature cardinality',
  ...
  output = output
      .replaceAll(
        '18개 동료 독립 장면·10개 활동 미니 이벤트',
        '18개 동료 독립 장면·36개 동행 선택·10개 활동 미니 이벤트',
      )
      .replaceAll(
        '18 companion scenes·10개 activity mini-events',
        '18 companion scenes·36 companion choices·10개 activity mini-events',
      )
      .replaceAll(
        'page 13 companion scene record ko/en',
  ...
  if (!output.contains('활동 horizon Golden')) {
    output = output.replaceAll(
      '활동 forecast Golden',
      '활동 forecast Golden·활동 horizon Golden',
    );
  }
