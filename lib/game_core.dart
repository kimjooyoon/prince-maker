...
class StoryChoiceMade extends GameEvent { const StoryChoiceMade(this.stat, this.delta, this.coins, this.label, {this.bondId, this.bondDelta = 0, this.requiresStat, this.requiresMin = 0}); final String stat, label; final int delta, coins, bondDelta, requiresMin; final String? bondId, requiresStat; }
...
class ProgressComponent { int week = 1; int coins = 12; int fatigue = 0; int selected = 0; int persona = 0; int eventIndex = 0; String lastResult = ''; final bonds = <String, int>{'lumi':0,'bora':0,'taro':0}; final milestones = <String, bool>{}; final trace = <String>[]; }
...
class GameSession {
  ...
  void choose(ActivityChosen e) { final p = world.progress[0]!, people = story.personalities; final person = people.isEmpty ? null : people[p.persona.clamp(0, people.length - 1)]; final bonus = person?['focusStat'] == e.stat ? (person?['focusBonus'] as int? ?? 0) : 0; world.dispatch(ActivityChosen(e.stat, e.delta, e.coins, e.fatigue, label: e.label, bonus: bonus)); if (world.progress[0]!.week < story.endingWeek) world.dispatch(const WeekAdvanced()); if (!story.events.any((e) => e['week'] == world.progress[0]!.week)) _resolveMilestone(); }
  void chooseEvent(StoryChoiceMade e) { final p = world.progress[0]!; if (e.requiresStat != null && (world.stats[0]!.values[e.requiresStat] ?? 0) < e.requiresMin) { p.lastResult = '조건 부족 · ${e.requiresStat} $e.requiresMin 필요'; return; } world.dispatch(e); _resolveMilestone(); }
