...
class GameSnapshot {
  ...
  Map<String, dynamic> toJson() => {'schema': schema, 'week': week, 'coins': coins, 'fatigue': fatigue, 'selected': selected, 'persona': persona, 'page': page, 'eventIndex': eventIndex, 'stats': stats, 'bonds': bonds, 'milestones': milestones, 'lastResult': lastResult, 'lastLine': lastLine, 'history': history};
  String get replayTrace => history.join('>');

  factory GameSnapshot.decode(String raw) {
    final j = jsonDecode(raw) as Map<String, dynamic>;
    if (j['schema'] != 'lumen-save-v3' && j['schema'] != 'lumen-save-v4' && j['schema'] != 'lumen-save-v5' && j['schema'] != 'lumen-save-v6') throw const FormatException('unsupported save schema');
